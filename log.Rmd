...
Logistic and bagging are tied for the highest accuracy, it might be worth it to try further experimentation with both of these algorithms. Especially to try and reduce the false negatives to below the NaiveBayes benchmark of 6.
...
The accuracy seems to have improved for the logistic algorithm, but there are still quite a few false negatives, something we wish to avoid. Bagging actually saw a decrease in performance.
...
By increasing the cost of false negatives even further we managed to increase the accuracy some more and reduce the amount of false negatives by 7, with the price of 2 additional false positives. This brings the total false negatives to even below NaiveBayes' impressive 6.
...
plt <- ggplot(data = roc_data,
    ...
    labs(y="True Positive Rate", x="False Positive Rate",
       caption="Figure 9: ROC-Curve of model using a cost-sensitive logistic classifier.") +
...
print(plt)
```
The ROC-Curve shown in figure 9 shows the performance of the model using a cost-sensitive logistic classifier with a valuation of 4 to 1. It shows that the model performs quite well and is slightly more sensitive than it is specific, which is exactly the compromise I set out to make during my evaluation of the ramifications of false positives and false negatives on the to-be-diagnosed patient.

Let's create some test data for the java wrapper:
```{r}
library(farff)
new_data <- clean_data
# Empty the class column so it can be refilled by the classifier.
new_data$Class <- NA
writeARFF(new_data, "test.arff")
```
The test data works as intended and gives proper output.
